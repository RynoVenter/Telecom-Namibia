...
public class IssueTransportOrder extends VtiUserExit
	
{
	public VtiUserExitResult execute() throws VtiExitException
	{
		...
		VtiUserExitScreenField stoI = getScreenField("C_STO_I");
		VtiUserExitScreenField vtiRef = getScreenField("VTI_REF");
		VtiUserExitScreenField trNo = getScreenField("WA_TRNO");
		VtiUserExitScreenField tranType = getScreenField("TRAN_TYPE_I");
		...
		if (stoI == null)	return new VtiUserExitResult(999, "The following screen element does not exist : C_STO_I");
		...
		if (tranType == null) return new VtiUserExitResult(999, "The following screen element does not exist : TRAN_TYPE_I");
		...
		if(tRowCount == 0)
			return new VtiUserExitResult(999,"No Items to Issue");
		...
		while(incCounter<tRowCount)
		{
			...
			if(chkItem.equals(itemRow.getFieldValue("IT_SEL")))
			{
				...
				if(isQty == 0)
					{
						return new VtiUserExitResult(999,"No unit to issue in line " + (incCounter + 1) + ".");
					}
		...
		// Select the required records.
			...
			if(headerLdbRows.length == 0)
				return new VtiUserExitResult(999,"No Transport Order found in the Header.");
			...
			issueHeaderLdbTable.saveRow(headerLdbRows[0]);//error if 2 supervisors try to save the same row.
			
			toNum = trNo.getStringFieldValue();
		}
		catch (VtiExitException ee)
		{
			Log.error("Error, reference the lof file please.", ee);
