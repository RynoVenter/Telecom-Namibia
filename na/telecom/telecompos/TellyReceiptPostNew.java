...
public class TellyReceiptPostNew extends VtiUserExit
{
	public VtiUserExitResult execute() throws VtiExitException
	{
		...
		VtiExitLdbTable YSPS_TELLY_RECEIPT_WORKING = getLocalDatabaseTable("YSPS_TELLY_RECEIPT_WORKING");
		...
		if (YSPS_TELLY_RECEIPT_WORKING== null)
			return new VtiUserExitResult(999, "LDB table YSPS_TELLY_RECEIPT_WORKING does not exist");
...
//		Get tax value
		...
		//Delete the temporary working table
		YSPS_TELLY_RECEIPT_WORKING.deleteAllRows();

		//check if there are any sales qty
		for(int b=0; b<AllocScrTable.getRowCount();b++)
		{
			VtiUserExitScreenTableRow allocScrTableRow = AllocScrTable.getRow(b);
			double dblSalesQty = allocScrTableRow.getDoubleFieldValue("SOLD");
			if (dblSalesQty>0)
			{
				blnSalesExist = true;
				b=AllocScrTable.getRowCount();
			}
		}

		System.out.println("Sales exist " + blnSalesExist);
		...
		for(int b=0;b<AllocScrTable.getRowCount();b++)
		{
			...
			double dblIssueQty =  allocScrTableRow.getDoubleFieldValue("QTY");
			...
			//Only perform if sales exist
			if(blnSalesExist)
			{

				intSalesItemNo += 1;
				
				VtiExitLdbTableRow newItemRow = YSPS_TELLY_RECEIPT_WORKING.newRow();
				newItemRow.setFieldValue("KUNNR", strCustomer);
				newItemRow.setFieldValue("TRAN_NO", strHistTranNo);
				newItemRow.setFieldValue("ITEM_NO", strHistItemNo);
				newItemRow.setFieldValue("MATNR",moveMatnr );
				newItemRow.setFieldValue("DESCRIPTION",moveMatnrDesc );
				newItemRow.setFieldValue("ISSUE_QTY",dblIssueQty );
				newItemRow.setFieldValue("RRP",String.valueOf(dblRRP));
				newItemRow.setFieldValue("DISCOUNT_PRICE",String.valueOf(dblDiscountPrice ));
				newItemRow.setFieldValue("SOLD",dblSoldQty );
				newItemRow.setFieldValue("RETURN",dblReturnQty );
				newItemRow.setFieldValue("PLANT_FR",strDefaultPlant );
				newItemRow.setFieldValue("STORE_FR", strCustomerStore);
				newItemRow.setFieldValue("STORE_TO",strSaleableStore );

				YSPS_TELLY_RECEIPT_WORKING.saveRow(newItemRow);
				...
				if(salesTranNo==0)
				{
					...
					System.out.println("Tran no" + salesTranNo);
				}
				
				
				//Document item record
				newItemRow = docItemsLdbTable.newRow();
			...
			else //Only returns, don't have to proceed with payment screen
			{
				...
				if(dblReturnQty>0)
				{
					...
					if (hostConnected)
					{ 
						VtiExitLdbRequest ldbReqUploadMvmtHeader = new VtiExitLdbRequest(YSPS_MVMT_HEADER,VtiExitLdbRequest.UPLOAD);
						VtiExitLdbRequest ldbReqUploadMvmtItems = new VtiExitLdbRequest(YSPS_MVMT_ITEMS,VtiExitLdbRequest.UPLOAD);
						VtiExitLdbRequest ldbReqUploadTellyDep = new VtiExitLdbRequest(YSPS_TELLY_DEP,VtiExitLdbRequest.UPLOAD);
						VtiExitLdbRequest ldbReqUploadTranQueue = new VtiExitLdbRequest(YSPS_TRAN_QUEUE,VtiExitLdbRequest.UPLOAD);
...
//		Create sales movement header
		...
		headerLdbRow.setFieldValue("KUNNR",strCustomer);
		...
		System.out.println("Tran no for header" + salesTranNo);
